...
if (!class_exists('CryptoCoin')) :
    class CryptoCoin
    {
        ...
        private function parse_data($dirty_data_as_array)
        {
            ...
            $this->coin_html = $this->raw ['coin_html'];
            $this->id = $this->raw ['id'];
        }
         ...
         */
        public function update_coin_post_meta($post_id)
        {
            $meta_values_to_set = array (
                '__coin_dimensions' => $this->raw['dimensions'],
            );
...
if (!class_exists('CryptoCoins')) :
    class CryptoCoins
    {
         ...
         */
        private static function GetCryptoCoins($url)
        {
            // remote data
         ...
         */
        public static function LoadRemoteCryptoCoins($to_create_posts)
        {
            self::$remote_data = self::GetCryptoCoins('https://www.cryptocompare.com/api/data/coinlist/');
            ...
            foreach (self::$remote_data ['rows'] as $inx => $remote_item_data) {
                ...
                $coin = new CryptoCoin($remote_item_data);
                $works[] = $coin;
            }
            ...
            if ($to_create_posts) {
                ...
                foreach (self::$coins as $inx => $coin) {
                    ...
                    $post_id = self::CreateLocalCryptoCoin($coin);
                    ...
                    if ($post_id == -1) {
                        ...
                        $success = $coin->update_coin_post_meta($post_id);
                    }
///////////////////////////////////////////////////
///////////////////////////////////////////////////
...
///////////////////////////////////////////////////
        ...
        public static function CreateCryptoCoin($coin)
        {
            if (!$coin) return -1;
            ...
            $img_url = $coin->raw['img_url'];
            $coin_html = $coin->coin_html;

            $content = "<p>${coin_html}</p>\n\r<br />\r\n<a target=\"_blank\" href=\"${img_url}\"><img width=\"100%\" height=\"auto\" src=\"${img_url}\"></a>";

            $new_post_id = self::CreateNewCryptoCoin($post_type, $coin->title, $content, $template, $coin);
         ...
         */
        public static function CreateNewCryptoCoin($post_type = 'post', $title, $content, $template_rel_path = '', $data)
        {
